...
annot_tags = ["genemark_on_40X_ref", "augustus_on_40X_ref", "v3_from_12Xv2_raw_on_40X_ref", "SRA_transcriptome2_psiclass_on_40X_ref", "SRA_transcriptome2_stringtie_default_on_40X_ref", "SRA_transcriptome2_stringtie_morus_on_40X_ref", "SRA_transcriptome1_stringtie_default_on_40X_ref", "SRA_transcriptome1_stringtie_morus_on_40X_ref", "stringtie_SRA1_default_STAR_2_on_40X_ref", "stringtie_SRA1_MorusCommands_STAR_2_on_40X_ref"]

annotations = {}

chosen_chromosome = "chr15"
...
location = f"{pickle_path}full_{chosen_chromosome}_abinitio_transcriptome_merge_on_40X_ref_annotation.pkl"
if os.path.isfile(location):
    b = pickle_load(location)
else:
    b.name = f"full_{chosen_chromosome}_abinitio_transcriptome_merge"
    b.id = f"full_{chosen_chromosome}_abinitio_transcriptome_merge_on_40X_ref"
    ...
    # b.add_blast_hits("Araport", f"{b.path}/blast_results/David_test3_{chosen_chromosome}_Araport11.diamond")
    # b.add_blast_hits("Viridiplantae", f"{b.path}/blast_results/David_test3_{chosen_chromosome}_Viridiplantae.diamond")
    # b.add_blast_hits("Eudicotyledons", f"{b.path}/blast_results/David_test3_{chosen_chromosome}_Eudicotyledons.diamond")
    b.update()
    b.export_gff(UTRs=True)
    pickle_save(location, b)

location = f"{pickle_path}{chosen_chromosome}_abinitio_transcriptome_merge_reduced_on_40X_ref_annotation.pkl"
if os.path.isfile(location):
    b = pickle_load(location)
else:
    b.name = f"{chosen_chromosome}_abinitio_transcriptome_merge_reduced"
    b.id = f"{chosen_chromosome}_abinitio_transcriptome_merge_reduced_on_40X_ref"
    b.calculate_transcript_masking(genomes[f"{b.genome}_hard"])
...
location = f"{pickle_path}{chosen_chromosome}_abinitio_transcriptome_merge_reduced_combined_on_40X_ref_annotation.pkl"
if os.path.isfile(location):
    b = pickle_load(location)
else:
    b.name = f"{chosen_chromosome}_abinitio_transcriptome_merge_reduced_combined"
    b.id = f"{chosen_chromosome}_abinitio_transcriptome_merge_reduced_combined_on_40X_ref"
