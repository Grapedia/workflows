...
parser.add_argument('--most_specific_id_level_for_feature_fastas', type=str, choices=["gene", "transcript", "CDS", "protein", "promoter"], help='The most specific id which is used in exported fastas.', required=False, default="gene")
...
def process_annotation(annot, genomes, dap, coordinates_path, gff_path, features_path, clean, featurecounts, lengths, coordinates, features, promoters, promoter_size, aegis, gffs, skip_atypical, utrs, combine, stats_path, stats, max_x, rename_chromosomes_based_on_config:bool, feature_id_level, fasta_header_details, conf_genomes:dict={}, dap_genomes:dict={}):
    ...
    if aegis:
        if a.name == "4.0_4.2_merge":
            a.rename_ids(remove_point_suffix=True)
        else:
            a.rename_ids()
    ...
    if not only_genomes:
        ...
        for k, v in annotation_files.items():
            ...
            if os.path.isfile(location):
                ...
                if objects != []:
                    if k in objects:
                        annotations[k] = Annotation(annot_name, v, genomes[genome], sort_processes=sort_processes)
            ...
            if objects != []:
                if k in objects:
                    if rename_chromosomes_based_on_config:
                        process_annotation(a, genomes, False, coordinates_path, gff_path, features_path, clean, featurecounts, lengths, coordinates, features, promoters, promoter_size, aegis, gffs, skip_atypical, utrs, combine, stats_path, export_stats, max_x, False, feature_id_level, fasta_header_details)
                    process_annotation(a, genomes, False, coordinates_path, gff_path, features_path, clean, featurecounts, lengths, coordinates, features, promoters, promoter_size, aegis, gffs, skip_atypical, utrs, combine, stats_path, export_stats, max_x, rename_chromosomes_based_on_config, feature_id_level, fasta_header_details, conf_genomes)
                    if dap and not a.dapfit:
                        process_annotation(a, genomes, dap, coordinates_path, gff_path, features_path, clean, featurecounts, False, coordinates, False, promoters, promoter_size, aegis, gffs, skip_atypical, utrs, combine, stats_path, export_stats, max_x, rename_chromosomes_based_on_config, feature_id_level, fasta_header_details, conf_genomes, dap_genomes)
        ...
        for k, v in annotation_transfer_files.items():
            ...
            if os.path.isfile(location):
                ...
                if objects != []:
                    if tag in objects:
                        transferred_annotations[tag] = Annotation(f"{annot_name}_from_{origin}", v, genomes[genome], annotations[f"{annot_name}_on_{origin}"], define_synteny=True, sort_processes=sort_processes)
