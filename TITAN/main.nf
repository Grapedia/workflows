...
workflow {
    ...
    def evidence_data_done = Channel.value(true)
    ...
    if (params.workflow == "generate_evidence_data" || params.workflow == "all") {
        ...
        evidence_data_done = evidence_data.done
        evidence_data_results = evidence_data.results
    } else {
      evidence_data_results = Channel.empty()
      evidence_data_done = Channel.value(true)
    }

    evidence_data_done
    .subscribe { println "Evidence data generation completed/existing, Aegis launched..." }
    ...
    // also if the workflow parameter is all, the outputs generated by the first workflow (generate_evidence_data) are retrieve manually from the first step
    if (params.workflow == "aegis" || params.workflow == "all") {
