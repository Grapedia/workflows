...
include { aegis_solo } from './subworkflows/aegis_solo.nf'
include { aegis_all } from './subworkflows/aegis_all.nf'
...
workflow {
    ...
    if (params.workflow == "generate_evidence_data") {
        println "Evidence data generation in progress ..."
        evidence_data = generate_evidence_data(
            samples_list_long_reads, 
            samples_list_short_reads, 
            protein_list
        )
        println "Evidence data generation process completed, you only requested this process without Aegis, TITAN ends."
      ...
      aegis_solo(workflow_inputs_list) // We send the list directly, not a Channel
      println "Aegis process completed, TITAN ends."
