...
// Same BUSCO run as above, on the "high confidence" protein set (unsupported
// single-exon genes removed by flag_low_confidence_monoexonic_genes) so its
// completeness can be compared side by side against the full annotation's.
// A distinct process (not a parameterized reuse of `busco`) so this never
// changes the task hash / cache of the already-validated primary busco run.
process busco_high_confidence_monoexonic {
  label 'process_aegis'
