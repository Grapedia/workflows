...
3. ✅ M3 partiel (2026-07-18) : `mikado configure`+`mikado prepare` (conteneur pinné `quay.io/biocontainers/mikado@sha256:dd6f5a2a...`, via `srun`+`apptainer exec`, hors Nextflow) exécutés réellement sur le génome T2T complet et les vraies évidences déjà produites par le run de production (Liftoff `liftoff_previous_annotations.gff3`, 518k features ; les deux GTF long-reads réels `merged_minimap2_stringtie_long_reads_{default,alt}.gtf`, 18 Mo + 10 Mo ; sources StringTie/PsiCLASS/HISAT2/BRAKER/EGAPx/Helixer non encore disponibles à ce stade du run, passées vides). Résultat : `mikado_prepared.fasta`/`.gtf` produits sans erreur, 61 905 transcrits consolidés (109 Mo / 98 Mo), "Mikado prepare has finished correctly". **Bug de données réel découvert et corrigé** (dans `modules/mikado.nf`, pas dans `scripts/make_mikado_list.py`) : `liftoff_previous_annotations.gff3` contient 3 lignes de notes de curateur Apollo en texte libre qui ont fuité comme lignes GFF3 mal formées (non tabulées à 9 colonnes) — le parseur strict de Mikado (et AGAT) abandonne tout le `prepare` dès la première rencontrée. Corrigé en filtrant défensivement `awk -F'\t' 'NF == 9 || /^#/'` sur `liftoff_gff3` avant utilisation. Reste non testé aujourd'hui : `mikado serialise --orfs` (a besoin du `.bed` TransDecoder, cf. Phase 5) et `mikado pick` sur ces données réelles — validation end-to-end complète nécessite ces deux étapes en plus, hors scope du temps disponible aujourd'hui.
...
4. ✅ Cross-check (2026-07-18) sur le vrai `mikado_prepared.fasta` produit par la Phase 4 M3 (61 905 transcrits réels) : run réel (mêmes conteneur/correctif que M2) sur un sous-ensemble de 2000 de ces transcrits. Résultat : 1737/2000 CDS prédits (87 %) — proportion nettement plus élevée qu'avec le lot v4 mixte de M2 (58 %), cohérent avec le fait que `mikado_prepared.fasta` est déjà un jeu de transcrits consolidés/filtrés par Mikado, donc plus enrichi en vrais gènes codants qu'un échantillon brut d'exons v4. Confirme que le correctif PATH (M2) tient sur des données de production réelles, pas seulement sur le fixture initial.
...
2. ✅ M2 (2026-07-18) : `flair align`/`correct`/`collapse` (conteneur pinné `quay.io/biocontainers/flair@sha256:187e2e22...`, via `srun`+`apptainer exec`, hors Nextflow) exécutés réellement sur 500 lectures long-read réelles du sample du projet (`hq_transcripts.RI_rmv_Antonio.fasta`) contre le génome T2T complet, avec correction par annotation Liftoff réelle. `flair align` : 500/500 alignements. **Deux bugs réels découverts et corrigés** : (1) `flair correct` de cette version n'a plus d'argument `-g`/genome (`unrecognized arguments`) — supprimé du module ; (2) `-f`/`--gtf` de `flair correct` exige strictement un GTF, pas le GFF3 Liftoff utilisé partout ailleurs dans TITAN — corrigé en ajoutant un nouveau process `modules/agat_convert_gff3_to_gtf.nf` (AGAT, conteneur déjà pinné `container_agat`) exécuté une fois sur `liftoff_previous_annotations.gff3` et câblé en amont de `flair_isoforms` dans `subworkflows/generate_evidence_data.nf` (remplace l'ancien passage direct du GFF3). Ce nouveau process réutilise le même filtrage défensif que Mikado (Phase 4) pour les notes de curateur Apollo mal formées. Re-testé en `-profile test -stub-run` dans le worktree isolé `codex-dev` : graphe complet OK avec `agat_convert_gff3_to_gtf` intégré.
...
2. ✅ M2 (2026-07-18) : run réel (conteneur pinné `quay.io/biocontainers/sqanti3@sha256:3bd6ec96...`, via `srun`+`apptainer exec`, hors Nextflow) de `sqanti3_qc.py` sur un GTF StringTie réel restreint à chr01 contre une référence chr01 réelle. Résultat : 1236 isoformes classifiées, rapport HTML (5,2 Mo) et fichiers `_classification.txt`/`_corrected.gtf`/`_junctions.txt` bien formés, run complet en 38,75 s. **Deux bugs réels découverts et corrigés dans `modules/sqanti3_qc.nf`** : (1) `sqanti3_qc.py` exige des flags nommés `--isoforms`/`--refGTF`/`--refFasta`, pas des arguments positionnels ; (2) `--refGTF` rejette strictement le GFF3 (`is not a GTF file. Abort!`) alors que la référence TITAN est en GFF3 partout ailleurs — corrigé par une conversion `gffread -T` (déjà présent dans le conteneur) avant l'appel ; (3) le binaire interne `gtfToGenePred-linux-x86_64` de ce conteneur pinné est lié à `libbz2.so.1` mais l'image ne fournit que `libbz2.so.1.0.8` sans le lien SONAME — corrigé en créant ce symlink localement et en le préfixant à `LD_LIBRARY_PATH` avant l'appel. Sans ces trois correctifs, l'étape échouait immédiatement en production dès `run_sqanti3 = true`.
