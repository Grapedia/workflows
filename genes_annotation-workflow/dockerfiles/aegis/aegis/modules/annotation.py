...
class Annotation():
    ...
    features["UTR"] = ["UTR", "three_prime_UTR", "five_prime_UTR", 
    ...
    def __init__(self, name:str, annot_file_path:str, genome:object, original_annotation:object=None, target:bool=False, to_overlap:bool=True, rework_CDSs:bool=True, chosen_chromosomes:list=None, chosen_coordinates:tuple=None, sort_processes:int=2, define_synteny=False):
        ...
        self.warnings = {"1bp_exon": [], "decreasing_coordinates": [],
        ...
        if self.features == ["nucleotide_to_protein_match"]:
            ...
            for ID, features in temp_protein_matches.items():
                ...
                ch_size = len(genome.chseqs[sorted_features[0].ch])
                ...
                for line in lines:
                    ...
                    # transcript subfeatures
                        ...
                        if coord[0] == coord[1] and n == 3:
                            print(f"{self.id} Warning: 1bp {ft} {ID}")
                                ...
                                if parent not in self.all_transcript_ids:
                                    if parent in self.chrs[ch]:
                                        ...
                                        if self.chrs[ch][parent].pseudogene:
                                            ...
                                            if (pseudo_t in self.chrs[ch][parent]
                                                .transcripts):
                                                if n == 4:
                                                    # CDS segments
                                                    (self.chrs[ch][parent]
    ...
    def update(self, original_annotation:object=None, rename_ids_minimal=False, extra_attributes:bool=False, genome:object=None, define_synteny:bool=False, sort_processes:int=2):
        ...
        now = time.time()
        lapse = now - start
        print(f"\nWhole update process for {self.id} annotation object "
    ...
    def generate_promoters(self, genome:object, promoter_size, from_ATG:bool=False, count_prom_size_from_TSS:bool=True):
        """
    ...
    def export_all_features(self, feature_output:str="main", promoters:bool=True, verbose:bool=True, path:str=""):
        """
        The "output" parameter can be both, main or all. This parameter only 
        affects transcript, CDS and protein sequences. If both is selected
        a "main features" file and and "all features" file will be produced.
        Silimarly the "include_gene_with_promoter" parameter can be "yes",
        "no" or "both".
        """
        start = time.time()
        self.export_genes(verbose, path)
        if feature_output == "main" or feature_output == "both":
            self.export_proteins(True, verbose, path)
            self.export_CDSs(True, verbose, path)
            self.export_transcripts(True, verbose, path)
            if promoters:
                self.export_promoters(True, verbose, path)
        if feature_output == "all" or feature_output == "both":
            self.export_proteins(False, verbose, path)
            self.export_CDSs(False, verbose, path)
            self.export_transcripts(False, verbose, path)
            if promoters:
                self.export_promoters(False, verbose, path)
        now = time.time()
        lapse = now - start
        print(f"Extracting {self.id} annotation features took "
              f"{round(lapse, 1)} seconds\n")
            
    def export_proteins(self, only_main:bool=True, verbose:bool=True, custom_path:str=""):
        """
        ...
        output_file += self.id
        if only_main:
            output_file +=  "_main_proteins.fasta"
        else:
            output_file += "_all_proteins.fasta"
        out = ""
            ...
            for g in genes.values():
                ...
                for c in final_cs:
                    if c.protein.summary_tag and verbose:
                        out += f">{c.protein.id}|{c.protein.summary_tag}"
                    else:
                        out += f">{c.protein.id}" 
                    if verbose:
    ...
    def export_unique_proteins(self, genome:object=None, custom_path:str=""):
        ...
        if not self.contains_protein_sequences:
            ...
            system(f"mkdir -p {output_file}")
            output_file += f"{self.id}_unique_proteins.fasta"
            out = ""
            all_protein_seqs = {}
            self.all_protein_ids = {}
            ...
            for sequence, protein_id in unique_sequences.items():
                x += 1
                if x != (len(unique_sequences.keys()) - 1):
                    out += f">{protein_id}\n{sequence}\n"
                else:
                    out += f">{protein_id}\n{sequence}"    
    ...
    def export_CDSs(self, only_main:bool=True, verbose:bool=True, custom_path:str=""):
        """
        ...
        output_file += self.id
        if only_main:
            output_file += "_main_CDSs.fasta"
        else:
            output_file += "_all_CDSs.fasta"
        out = ""
        f_out = open(output_file, "w", encoding="utf-8")
        for genes in self.chrs.values():
            for g in genes.values():
    ...
    def export_transcripts(self, only_main:bool=True, verbose:bool=True,
                           custom_path:str=""):
        """
        ...
        output_file += self.id
        if only_main:
            output_file += "_main_transcripts.fasta"
        else:
            output_file += "_all_transcripts.fasta"
        out = ""
        f_out = open(output_file, "w", encoding="utf-8")
        for genes in self.chrs.values():
            for g in genes.values():
    ...
    def export_promoters(self, only_main:bool=True, verbose:bool=True, custom_path:str=""):
        if custom_path == "":
            output_path = self.path + "features/"
            output_file = output_path 
        else:
        ...
        if only_main:
            output_file += self.id + "_main_promoters"
        else:
            output_file += self.id + "_all_promoters"
                ...
                for t in g.transcripts.values():
                    if only_main:
                        if t.main:
                            if hasattr(t, "promoter"):
                                if t.promoter.seq != "":
                                    out += f">{g.id}_promoter|{t.promoter.type}"
                                    if verbose:
                                        out += (f"|{t.strand}|{t.ch}|"
                                                f"{t.promoter.start}:{t.promoter.end}")
                                    out += f"\n{t.promoter.seq}\n"
                                else:
                        ...
                        if hasattr(t, "promoter"):
                            if t.promoter.seq != "":
                                out += f">{t.promoter.id}|{t.promoter.type}"
                                if verbose:
                                    out += (f"|{t.strand}|{t.ch}|"
                                            f"{t.promoter.start}:{t.promoter.end}")
                                out += f"\n{t.promoter.seq}\n"
    ...
    def update_stats(self, custom_path:str="", export:bool=False, genome:object=None, max_x:int=None):
        ...
        self.stats = {"mean_transcripts" : [], "mean_exons" : [], "mean_gene_size" : [], "mean_intron_size" : [], "mean_CDS_size" : [], "mean_UTR_size" : [], "mean_transcript_size" : []}
        ...
        if export:
            for key in self.stats:
                if "mean" in key:
                    ...
                    if tag[-1] != "s":
                        tag += "s"
                    barplot(self.stats[key], export_folder, f"{self.id}_{tag}", f"Distribution of {self.id} {tag}", max_x)   
        ...
        if genome != None:
            ...
            if export:
                ...
                pie_chart(mod_labels, values, export_folder, f"{self.id}_intron_composition", f"Intron composition of {self.id} annotation")
        ...
        if export:
            if genome != None:
                out_file = f"{self.id}_full_stats.csv"
            else:
    ...
    def export_equivalences(self, custom_path:str="", stringent:bool=True, verbose:bool=True, return_df:bool=True, NAs:bool=True, export_csv:bool=False, export_self:bool=False):
        ...
        if export_self:
            ...
            export = "other"
            export_tag = ""
        tag = f"{export_tag}{self.id}_equivalences"
        if stringent:
            acceptable_values = [6, 7, 8, 9, 10, 11]
            tag += "_filtered"
        else:
    ...
    def export_lengths(self, just_gene:bool=True, custom_path:str=""):
        """
            ...
            for genes in self.chrs.values():
                for g in genes.values():
                    for t in g.transcripts.values():
                        if t.main:
                            out += f"{g.id}\t{t.size}\n"
            tag = f"{self.id}_gene_lengths_all.tsv"
        else:
            for genes in self.chrs.values():
                for g in genes.values():
                    if not g.pseudogene and not g.transposable:
                        for t in g.transcripts.values():
                            if t.main:
                                out += f"{g.id}\t{t.size}\n"
            tag = f"{self.id}_gene_lengths.tsv"
    ...
    def export_coordinates(self, custom_path:str="", lengths:bool=False):
        ...
        if self.dapmod:
            tag = f"{self.id}_gene_coordinates_dapmod.tsv"
        else:
            tag = f"{self.id}_gene_coordinates.tsv"
        f_out = open(f"{export_folder}{tag}", "w", encoding="utf-8")
        f_out.write(out)
        f_out.close()
    ...
    def remove_noncoding_transcripts_from_coding_genes(self):
        for genes in self.chrs.values():
            for g in genes.values():
                if g.coding and g.noncoding:
                    new_transcripts = {}
                    for t in g.transcripts.values():
                        if t.coding:
                            new_transcripts[t.id] = t.copy()
                    t.transcripts = new_transcripts.copy()
        self.update()
    ...
    def rename_ids(self, minimal:bool=False, prefix:str="", suffix:str="", spacer:int=100, from_merge:bool=False, extra_attributes:bool=False, remove_point_suffix:bool=False, correspondences:bool=False, custom_path=""):
        """
        ...
        if correspondences:
            ...
            for k, v in correspondences_d.items():
                out_text += f"{v}\t{k}\n"
            f_out = open(f"{export_folder}{self.id}_rename_eqs.tsv", "w", encoding="utf-8")
    ...
    def rename_chromosomes(self, equivalences):
