...
class Feature():
    ...
    def generate_sequence(self, genome:object):
        if self.start != "NA" and self.end != "NA":
            if self.strand == "+":
                self.seq = genome.chseqs[self.ch][self.start-1:self.end]
            elif self.strand == "-":
                self.seq = reverse_complement(genome.chseqs[self.ch]
                                            [self.start-1:self.end])
            elif self.strand == ".":
                self.seqs = (genome.chseqs[self.ch][self.start-1:self.end], reverse_complement(genome.chseqs[self.ch][self.start-1:self.end]))
    ...
    def generate_hard_sequence(self, hard_masked_genome:object):
        if self.start != "NA" and self.end != "NA":
            if self.strand == "+":
                self.hard_seq = hard_masked_genome.chseqs[self.ch][self.start-1:self.end]
            elif self.strand == "-":
                self.hard_seq = reverse_complement(hard_masked_genome.chseqs[self.ch]
                                            [self.start-1:self.end])
            elif self.strand == ".":
                self.hard_seqs = (hard_masked_genome.chseqs[self.ch][self.start-1:self.end], reverse_complement(hard_masked_genome.chseqs[self.ch][self.start-1:self.end]))
