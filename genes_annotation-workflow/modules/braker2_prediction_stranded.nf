...
process braker2_prediction_stranded {
  ...
  containerOptions "--user \$(id -u):\$(id -g) --volume ${protein_samplesheet_path}:/protein_samplesheet_path --volume ${projectDir}/scripts:/scripts --volume ${projectDir}/work:/work --volume ${projectDir}/data/protein_data:/protein_path --volume ${genome_path}:/genome_path --volume $params.outdir/evidence_data/RNAseq_stranded/alignments/new_assembly:/alignments --volume ${projectDir}:/outdir:z"
  ...
  script:
    ...
    --prot_seq=\${proteins} \
    --cores=${task.cpus} --workingdir=/outdir/TMP_stranded --etpmode --softmasking --gff3 \
