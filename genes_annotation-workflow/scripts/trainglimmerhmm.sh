...
usage() {
  cat <<-__EOF__
    Usage:
      ./trainglimmerhmm.sh -a assembly -t transcriptome -d output_dir [-h]
    ...
    Exemple: ./trainglimmerhmm.sh -a assembly.fasta -t transcriptome.gff3 -d training_dir
		__EOF__
}

# Eval command line arguments given in input
ARGS=$(getopt -o "a:t:d:h" --long "assembly:,transcriptome:,dir:,help" -- $@ 2> /dev/null)
...
do
  mkdir -p intermediate_files/GlimmerHMM/
  transcript_subset=$(bc <<< ${transcript_number}/${i}-1)

  # Extract random transcripts from transcriptome
  scripts/extract_exons_random_transcripts.py -i ${TRANSCRIPTOME} -o intermediate_files/GlimmerHMM/exons.tsv -n ${transcript_subset}

  # Train GlimmerHMM
  trainGlimmerHMM ${ASSEMBLY} intermediate_files/GlimmerHMM/exons.tsv -d ${DIR}
  ...
  then
    ...
    rm -r intermediate_files/GlimmerHMM/
  fi
done
